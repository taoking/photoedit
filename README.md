...
当前已完成 Phase 1 至 Phase 8.5（含最终正确性审查修复）。功能包含基础调色、LUT、HSL、曲线、预设、基于 `CIRAWFilter` 的 DNG/ARW RAW 调整、批量处理、色彩管理、HDR preview/SDR tone mapping/10-bit HEIF HDR 导出、线性/径向/画笔局部调整，以及独立的视频 LUT 预览与导出工作流。阶段状态以 [development-progress.md](docs/development-progress.md) 为准。
