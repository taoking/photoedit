...
SDR HSL 在单个 Core Image kernel 中完成八通道分类与合成：先对 extended-linear 工作值执行显式 sRGB transfer，再基于同一原始像素计算所有通道权重，最后恢复线性值和原有 headroom。所有滑杆状态按界面显示精度量化，因此显示为 0 的参数在渲染、预设、复制和会话中也是精确中性值。
